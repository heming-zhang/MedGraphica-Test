...
def build_geogat_model(args, device, graph_output_folder, num_class):
    ...
    # Build up model
    model = GATDecoder(kg_emb_dim=args.kg_emb_dim, input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, 
                    node_num=node_num, num_head=args.num_head, device=device, num_class=num_class)
    model = model.to(device)
    return model


def train_geogat_model(dataset_loader, num_graph, entity_embeddings, relation_embeddings, model, device, args, learning_rate):
    optimizer = torch.optim.Adam(filter(lambda p : p.requires_grad, model.parameters()), lr=learning_rate, eps=1e-7, weight_decay=1e-10)
    batch_loss = 0
    batch_entity_embeddings = np.tile(entity_embeddings, (num_graph, 1))
    batch_relation_embeddings = np.tile(relation_embeddings, (num_graph, 1))
    batch_entity_embeddings_tensor = torch.tensor(batch_entity_embeddings, requires_grad=False).to(device)
    batch_relation_embeddings_tensor = torch.tensor(batch_relation_embeddings, requires_grad=False).to(device)
    # import pdb; pdb.set_trace()
    for batch_idx, data in enumerate(dataset_loader):
        ...
        output, ypred = model(x, batch_entity_embeddings_tensor, batch_relation_embeddings_tensor, edge_index)
...
def train_geogat(args, fold_n, load_path, iteration_num, device, graph_output_folder, num_class):
    ...
    entity_embeddings = np.load(embed_folder_path + '/entity_embeddings.npy')
    relation_embeddings = np.load(embed_folder_path + '/relation_embeddings.npy')
    ...
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            model, batch_loss, batch_ypred = train_geogat_model(dataset_loader, num_graph, entity_embeddings, relation_embeddings, model, device, args, learning_rate)
...
def test_geogat_model(dataset_loader, num_graph, entity_embeddings, relation_embeddings, model, device, args):
    batch_loss = 0
    batch_entity_embeddings = np.tile(entity_embeddings, (num_graph, 1))
    batch_relation_embeddings = np.tile(relation_embeddings, (num_graph, 1))
    batch_entity_embeddings_tensor = torch.tensor(batch_entity_embeddings, requires_grad=False).to(device)
...
def test_geogat(args, fold_n, model, test_save_path, device, graph_output_folder, i):
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        print('TEST MODEL...')
        # import pdb; pdb.set_trace()
        model, batch_loss, batch_ypred = test_geogat_model(dataset_loader, num_graph, entity_embeddings, relation_embeddings, model, device, args)
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
